...
export default function EditProfileScreen({ navigation }) {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      const { error } = await supabase.storage.from('avatars').upload(path, { uri }, { upsert: true });
      if (!error) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const updates = { name: name.trim(), phone: phone.trim(), upi_id: upiId.trim(), avatar_url: avatar };
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.phoneRow}>
          ...
          <TextInput style={[styles.input, { flex: 1, marginBottom: 0 }]} value={phone} onChangeText={t => setPhone(t.replace(/\D/g, ''))} placeholder="9876543210" placeholderTextColor={COLORS.textMuted} keyboardType="phone-pad" maxLength={10} />
        </View>

        <Text style={[styles.label, { marginTop: SPACING.md }]}>UPI ID</Text>
        <TextInput style={styles.input} value={upiId} onChangeText={setUpiId} placeholder="yourname@upi" placeholderTextColor={COLORS.textMuted} autoCapitalize="none" />
        <Text style={styles.hint}>Setting a UPI ID lets friends pay you directly</Text>
      </View>
    </SafeAreaView>
  );
}
