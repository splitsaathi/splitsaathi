...
import { sendWhatsAppInvite, sendEmailInvite } from '../../services/contacts';
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.quickRow}>
        {[
          { icon: '📱', label: 'Contacts', color: '#6366f1' },
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {friends.length === 0 ? (
          <EmptyState
            emoji="👥"
            text="No friends yet.&#10;Add from Contacts, WhatsApp or Email!"
          ...
          filtered.map(f => (
            <View key={f.id} style={styles.friendCard}>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {f.phone && (
                  <TouchableOpacity style={styles.iconBtn} onPress={() => sendWhatsAppInvite(f.phone, profile.name)}>
                    <Text style={{ fontSize: 16 }}>💬</Text>
                  </TouchableOpacity>
                )}
                {f.email && (
                  <TouchableOpacity style={styles.iconBtn} onPress={() => Linking.openURL(`mailto:${f.email}`)}>
                    <Text style={{ fontSize: 16 }}>📧</Text>
                  </TouchableOpacity>
                )}
              </View>
