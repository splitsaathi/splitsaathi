...
import { getPhoneContacts, sendWhatsAppInvite, sendEmailInvite, sendSMSInvite } from '../../services/contacts';
...
export default function AddFriendScreen({ route, navigation }) {
  ...
  // Contacts tab
  const [contactsPerm, setContactsPerm] = useState('prompt'); // prompt | granted | denied
          ...
          <View>
            {contactsPerm === 'prompt' && (
              <View style={styles.permBox}>
                ...
                <Text style={styles.permDesc}>
                  ...
                  Your privacy is safe — contacts stay on your device only.
                </Text>
                <View style={styles.permInfo}>
                  <Text style={styles.permInfoTitle}>The app needs permission to:</Text>
                  {['📖 Read your contacts', '🔍 Find app members'].map(p => (
                    <Text key={p} style={styles.permInfoItem}>✓ {p}</Text>
                  ))}
                </View>
                <TouchableOpacity
            ...
            {contactsPerm === 'denied' && (
              <View style={styles.permBox}>
                ...
                <Text style={styles.permDesc}>
                  Permission was denied. Allow Contacts access from Phone Settings.
                </Text>
                <TouchableOpacity style={styles.permBtn} onPress={() => setTab('search')}>
                  <Text style={styles.permBtnText}>Add via Search</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {contactsPerm === 'granted' && (
              <View>
                ...
                <View style={styles.contactsInfo}>
                  <Text style={styles.contactsInfoText}>{filteredContacts.length} contacts</Text>
                  {selected.length > 0 && (
                    <Text style={{ color: COLORS.primary, fontWeight: '700', fontSize: 12 }}>{selected.length} selected</Text>
                  )}
                </View>
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ alignItems: 'flex-end', gap: 4 }}>
                        ...
                        {c.appUser && added.has(c.appUser.id)
                          ? <Text style={{ color: COLORS.textMuted, fontSize: 12 }}>Already added</Text>
          ...
          <View>
            ...
            <View style={styles.phoneRow}>
              ...
              <TextInput
                ...
                value={waNum} onChangeText={t => setWaNum(t.replace(/\D/g, ''))} maxLength={10} keyboardType="phone-pad"
              />
            </View>
            <Text style={{ color: COLORS.textMuted, fontSize: 13, marginTop: 8, marginBottom: SPACING.lg }}>
              A ready-made invite message will be sent
            </Text>
            <TouchableOpacity
              style={styles.waBtn}
              onPress={() => {
                if (waNum.length < 10) { Alert.alert('Error', 'Please enter a 10-digit number!'); return; }
                sendWhatsAppInvite(waNum, profile.name);
              }}
            >
            ...
            <View style={styles.divider}>
              ...
              <Text style={{ color: COLORS.textMuted, fontSize: 12, marginHorizontal: 12 }}>or share a group link</Text>
              <View style={{ flex: 1, height: 1, backgroundColor: COLORS.border }} />
            </View>
          ...
          <View>
            <View style={styles.waHeader}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={styles.waDesc}>Send an invitation to any email</Text>
              </View>
            </View>
            ...
            <View style={styles.previewBox}>
              ...
              <Text style={{ color: COLORS.textSub, fontSize: 13, lineHeight: 20 }}>
                <Text style={{ color: COLORS.text, fontWeight: '700' }}>{profile?.name}</Text> invited you{' '}
            ...
            <TouchableOpacity
              style={styles.waBtn}
              onPress={() => {
                if (!emailInv.includes('@')) { Alert.alert('Error', 'Please enter a valid email!'); return; }
                sendEmailInvite(emailInv, profile.name);
              }}
            >
...
const styles = StyleSheet.create({
  ...
  permInfo:   { backgroundColor: 'rgba(99,102,241,0.1)', borderRadius: RADIUS.md, padding: 12, width: '100%', marginBottom: SPACING.md },
  permInfoTitle: { color: COLORS.info, fontWeight: '700', fontSize: 13, marginBottom: 6 },
  permInfoItem:  { color: COLORS.textSub, fontSize: 13, marginBottom: 4 },
  ...
  contactsInfo:{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  contactsInfoText: { color: COLORS.textMuted, fontSize: 12 },
  ...
  checkbox:   { width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: COLORS.textDisabled, alignItems: 'center', justifyContent: 'center' },
  ...
  linkText:   { color: COLORS.textSub, fontSize: 13 },
