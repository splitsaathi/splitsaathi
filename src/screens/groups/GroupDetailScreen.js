...
export default function GroupDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    loadBills(group.id, profile.id);
    if (profile?.id) loadFriends(profile.id);
  }, [group.id]);
  ...
  const handleAddMember = async (friend) => {
    const alreadyMember = members.find(m => m.id === friend.id);
    if (alreadyMember) {
      Alert.alert('Already Member', `${friend.name} is already in this group!`);
      return;
    }
    setAddingMember(friend.id);
    const { error } = await addMemberToGroup(group.id, friend.id, profile.id);
    if (error) {
      Alert.alert('Error', error.message || 'Could not add member');
    } else {
      await loadGroupMembers(group.id);
      Alert.alert('✅ Added!', `${friend.name} added to ${group.name}!`);
    }
      ...
      <SafeAreaView edges={['top']} style={[s.headerSafe, { backgroundColor: COLORS.primary }]}>
        <View style={s.headerNav}>
          ...
          <View style={{ flexDirection:'row', gap:10 }}>
            {/* Add Member Button */}
            <TouchableOpacity style={[s.circleBtn, { backgroundColor:'rgba(255,255,255,0.2)' }]} onPress={() => setShowAddMember(true)}>
              <Text style={s.circleBtnText}>👥</Text>
            </TouchableOpacity>
        ...
        {/* Members count */}
        <Text style={{ color:'rgba(255,255,255,0.6)', fontSize:12, paddingHorizontal: SPACING.md, marginBottom:4 }}>
          👥 {members.length} members · Tap 👥 to add more
        </Text>
        ...
        {activeTab === 'stats' && (
          <View style={{ padding: SPACING.md, paddingBottom:40 }}>
            <View style={s.statsCard}>
              ...
              <View style={[s.thresholdBadge, { backgroundColor: exceedsThreshold? '#fef2f2':'#f0fdf4', borderColor: exceedsThreshold? '#fecaca':'#bbf7d0' }]}>
                ...
                <Text style={{ color: exceedsThreshold? COLORS.owe:'#15803d', fontSize:12, fontWeight:'600', flex:1 }}>
                  {exceedsThreshold ? `Warning! Expenses reached ${utilizationPct}% of your limit.` : `Safe! Budget utilizes ${utilizationPct}% of threshold.`}
                </Text>
              </View>
              <View style={[s.budgetBarBg, { marginTop:12 }]}>
                <View style={[s.budgetBarFill, { width:`${Math.min(100,utilizationPct)}%`, backgroundColor: utilizationPct>90? COLORS.owe: utilizationPct>80? '#f59e0b': COLORS.primary }]} />
              </View>
            </View>
            ...
            <View style={{ flexDirection:'row', gap:12, marginTop:4 }}>
              ...
              <TouchableOpacity style={[s.fabBtn, { flex:1, backgroundColor:'transparent', borderWidth:1.5, borderColor: COLORS.primary }]} onPress={() => setShowScanner(true)}>
        ...
        <View style={s.modalBackdrop}>
          <View style={[s.modalContent, { maxHeight:'70%' }]}>
            <View style={s.modalHeader}>
              <Text style={s.modalTitle}>👥 Add Member to {group.name}</Text>
              <TouchableOpacity onPress={() => setShowAddMember(false)}>
                <Text style={{ fontSize:20, color: COLORS.textMuted }}>✕</Text>
              </TouchableOpacity>
            </View>
            <Text style={{ color: COLORS.textMuted, fontSize:12, marginBottom:12 }}>
              {members.length} current member{members.length !== 1 ? 's' : ''} · Select from your friends
            </Text>
            {friends.length === 0 ? (
              ...
              <ScrollView>
                {friends.map(f => {
                  const isMember = !!members.find(m => m.id === f.id);
                  const isAdding = addingMember === f.id;
                  return (
                    <View key={f.id} style={{ flexDirection:'row', alignItems:'center', padding:14, borderBottomWidth:1, borderBottomColor: COLORS.borderLight }}>
                      <Avatar name={f.name} size={40} uri={f.avatar_url} />
                      <View style={{ flex:1, marginLeft:12 }}>
                        <Text style={{ color: COLORS.text, fontWeight:'600', fontSize:14 }}>{f.name}</Text>
                        <Text style={{ color: COLORS.textMuted, fontSize:12 }}>{f.email}</Text>
                      </View>
                      {isMember ? (
                        <View style={{ backgroundColor:'rgba(16,185,129,0.1)', paddingHorizontal:12, paddingVertical:6, borderRadius:20 }}>
                          <Text style={{ color:'#10b981', fontSize:12, fontWeight:'700' }}>✓ Member</Text>
                        </View>
                      ) : (
                        <TouchableOpacity
                          style={{ backgroundColor: isAdding ? 'rgba(16,185,129,0.1)' : COLORS.primary, paddingHorizontal:14, paddingVertical:8, borderRadius:20 }}
          ...
          <View style={s.modalContent}>
            ...
            <View style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:20 }}>
              {[['Stay','🏨'],['Travel','🚕'],['Food','🍔'],['Other','📦']].map(([cat,icon]) => (
                <TouchableOpacity key={cat} style={[s.catChip, expenseCategory===cat && { backgroundColor: COLORS.primary, borderColor: COLORS.primary }]} onPress={() => setExpenseCategory(cat)}>
                  <Text style={[{ fontSize:11, color: COLORS.textSub, fontWeight:'700' }, expenseCategory===cat && { color:'#fff' }]}>{icon} {cat}</Text>
                </TouchableOpacity>
              ))}
            </View>
      ...
      <Modal visible={showScanner} animationType="slide" transparent>
        <View style={s.modalBackdrop}>
          <View style={[s.modalContent, { maxHeight:'80%' }]}>
            ...
            <Text style={{ color: COLORS.textMuted, fontSize:12, marginBottom:16 }}>AI/OCR scan to auto extract & split bill lines</Text>
            {scanState === 'idle' && (
